...
def preprocess_continuous_features(feature_fp, df_filt):
...
def preprocess_features(feature_fp, df_filt):
   ...
   # print("Final drop list", final_drop_list)
    ...
    feature_preprocess_pipeline = Pipeline(steps=[
            ...
            #inna-updated
            # Missing value indicator for categoricals and continuous variables
            ('missing_indicator', AddMissingIndicator(variables=category_list)),
            #('missing_indicator', AddMissingIndicator(variables=category_list + continuous_list)),
    
            # Missing value imputation
                # Impute missing values with 0 for discrete variables
                ('arbitrary_number_imputer', ArbitraryNumberImputer(arbitrary_number = 0, variables = discrete_list)),   
    #            ('arbitrary_number_imputer', ArbitraryNumberImputer(arbitrary_number = 0, variables = discrete_list + ["con_service_hospitalist_service_count"])),
...
def run_xgb_tune_fit_1b(X_train, y_train, num_eval):
    ...
    best_xgb_model = xgb.XGBClassifier(**best_params, random_state=SEED)
...
def run_xgb_continuous_and_plot(df_filt, fp, output_folder, df_name, suffix, model_path):
    X_train_preprocessed, X_val_preprocessed, y_train, y_val = preprocess_continuous_features(fp, df_filt)
