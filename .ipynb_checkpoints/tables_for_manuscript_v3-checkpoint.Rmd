...
raw_master = fread("/gpfs/milgram/project/rtaylor/imc33/LOS/data/master_los.csv")
...
map_drg <- function(value) {
  dplyr::case_when(
    ...
    value %in% c("heart failure & shock w mcc", 
                 "Pulmonary edema & respiratory failure") ~ "Heart/Respiratory Failure (MCC)",
    ...
    value %in% c("Infectious & parasitic diseases w o.r. procedure w mcc",
                 ...
                 "Septicemia or severe sepsis w/o mv >96 hours w mcc") ~ "Sepsis (MCC/No MCC)",
...
map_medicine = function(value){
  case_when(value %in% c("Cardiology","Medicine") ~ "Medicine", 
            TRUE ~ value)
}
```
...
master = raw_master %>%
  mutate(viz_drg = map_drg(viz_drg),
         viz_service_collapsed = map_medicine(viz_service_collapsed))
...
descending_drg_order = master %>%
  group_by(viz_drg) %>%
  count() %>%
  arrange(desc(n)) %>%
  pull(viz_drg)

# Convert `viz_drg` to a factor using the descending order defined above
master$viz_drg <- factor(master$viz_drg, levels = descending_drg_order)

# Generate the table using table1, sorted as per the defined order
...
fname = paste0("/gpfs/milgram/project/rtaylor/imc33/LOS/output/tables_",formatted_date,".docx")
